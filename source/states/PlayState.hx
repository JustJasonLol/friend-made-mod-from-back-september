...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		timeTxt = new FlxText(STRUM_X + (FlxG.width / 2) - 248, (FlxG.height * (!ClientPrefs.data.downScroll ? 0.98 : 0.03)) + 40, 400, "", 32);
		timeTxt.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		timeTxt.scrollFactor.set();
		timeTxt.alpha = 0;
		timeTxt.borderSize = 2;
		timeTxt.visible = updateTime = showTime;
		if(ClientPrefs.data.downScroll) timeTxt.y -= 80;
		if(ClientPrefs.data.timeBarType == 'Song Name') timeTxt.text = '';
		...
		healthBar = new Bar(0, FlxG.height * (!ClientPrefs.data.downScroll ? 0.98 : 0.03), 'healthBar', function() return health, 0, 2);
	...
	public function startCountdown()
	{
		...
		if(ret != LuaUtils.Function_Stop) {
			...
			{
				if (!ClientPrefs.data.middleScroll) player.x += 80;
				if (!ClientPrefs.data.downScroll) player.y -= 70; else player.y += 70;
			}

			for (dad in opponentStrums)
			{
				if (!ClientPrefs.data.middleScroll) dad.x -= 80;
				if (!ClientPrefs.data.downScroll) dad.y -= 70; else dad.y += 70;
			}
	...
	function startSong():Void
	{
		...
		if(autoUpdateRPC) DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", 'kei', true, songLength);
		#end
	...
	{
		#if DISCORD_ALLOWED
		if (health > 0 && !paused && autoUpdateRPC) DiscordClient.changePresence(detailsPausedText, SONG.song + " (" + storyDifficultyText + ")", 'kei');
		#end
	...
	function resetRPC(?showTime:Bool = false)
	{
		...
		if (showTime)
			DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", 'kei', true, songLength - Conductor.songPosition - ClientPrefs.data.noteOffset);
		else
			DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", 'kei');
		#end
	}
	...
	override public function update(elapsed:Float)
	{
		...
		if (camZooming)
		{
			FlxG.camera.zoom = FlxMath.lerp(defaultCamZoom, FlxG.camera.zoom, .95);
			camHUD.zoom = FlxMath.lerp(.8, camHUD.zoom, .95);
		}
	...
	function openPauseMenu()
	{
		...
		#if DISCORD_ALLOWED
		if(autoUpdateRPC) DiscordClient.changePresence(detailsPausedText, SONG.song + " (" + storyDifficultyText + ")", 'kei');
		#end
	}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead && gameOverTimer == null)
		{
			...
			if(ret != LuaUtils.Function_Stop)
			{
				...
				{
					FlxTween.tween(vocals, {pitch: .001}, .9, {onComplete: d -> vocals.stop()});
					FlxTween.tween(FlxG.sound.music, {pitch: .001}, .9, {onComplete: d -> FlxG.sound.music.stop()});
					openSubState(new GameOverSubstate(boyfriend));
				}
				...
				if(autoUpdateRPC) DiscordClient.changePresence("Game Over - " + detailsText, SONG.song + " (" + storyDifficultyText + ")", 'kei');
				#end
				isDead = true;
				return true;
			}
		...
		if(ret != LuaUtils.Function_Stop && !transitioning)
		{
			...
			if (isStoryMode)
			{
				...
				else
				{
					...
					LoadingState.loadAndSwitchState(new EndDialogueState(), false, false);
				}
			}
			else
			{
